namespace SistemaDocumental {
    ...
    export function CrearCrudDeLotesContables(idPanelMnt: string, idPanelCreacion: string, idPanelEdicion: string, idModalBorrar: string) {
        Crud.crudMnt = new SistemaDocumental.CrudDeLotesContables(idPanelMnt, idPanelCreacion, idPanelEdicion, idModalBorrar);
    ...
    export class CrudDeLotesContables extends Crud.CrudMnt {
        ...
        constructor(idPanelMnt: string, idPanelCreacion: string, idPanelEdicion: string, idModalBorrar: string) {
            ...
            this.crudDeCreacion = new CrudCreacionLoteContable(this, idPanelCreacion);
            this.crudDeEdicion = new CrudEdicionLoteContable(this, idPanelEdicion);
        }
        ...
        protected DespuesDeInicializarCrud(modoAccesoAlNegocio: ModoAcceso.enumModoDeAccesoDeDatos) {
            ...
            ApiListaDinamica.AsignarValor(ctrlTipo, this.MapIndicadores.get(ltrPropiedades.SisDoc.CircuitosDoc.Indicadores.IdTipoLoteContable),
                this.MapIndicadores.get(ltrPropiedades.SisDoc.CircuitosDoc.Indicadores.TipoLoteContable), true);
        }
        ...
        protected FiltrosExpecificosParaCargarElGrid(operacion: string, clausulas: ClausulaDeFiltrado[]): ClausulaDeFiltrado[] {
            ...
            if (!clausulas.find(c => c.clausula === ltrPropiedades.Elemento.ConTipo.IdTipo)) {
                ...
                let valor = this.MapIndicadores.get(ltrPropiedades.SisDoc.CircuitosDoc.Indicadores.IdTipoLoteContable);
        ...
        public DespuesDeProcesarOpcionMf(peticion: ApiDeAjax.DescriptorAjax): boolean {
            ...
            if (opcion === ltrMenus.eventosDeMf.Contabilidad.Preasientos.CrearLoteContable) {
                this.crudDeEdicion.Expansor_AbrirModalParaPedirDatos((Crud.crudMnt as CrudDeLotesContables).ModalContabilizar.id, 0);
                return true;
            }
        }
        ...
        public ModalDePedirDatos_TrasAbrir(modal: HTMLDivElement): void {
            ...
            if (modal.id === this.ModalContabilizar.id) {
                ...
                var check = ApiControl.BuscarCheck(this.ModalContabilizar, ltrPropiedades.Contabilidad.CrearLote.Descontabilizar);
                ApiControl.OcultarControlSi(check, true);
            }
        }
        ...
        public ModalDePedirDatos_Aceptar(modal: HTMLDivElement) {
            ...
            if (modal.id === (Crud.crudMnt as CrudDeLotesContables).ModalContabilizar.id) {
    ...
    export class CrudCreacionLoteContable extends Crud.CrudCreacion {
    ...
    export class CrudEdicionLoteContable extends Crud.CrudEdicion {
        ...
        public DespuesDeProcesarOpcionMf(peticion: ApiDeAjax.DescriptorAjax) : boolean {
            ...
            let opcion: string = ObtenerPropiedad(peticion.DatosDeEntrada, ltrMenus.opcion);
            if (opcion === ltrMenus.eventosDeMf.Contabilidad.Preasientos.AnularLoteContable) {
                MensajesSe.Info('Trabajo de anulación de lote contable sometido correctamente');
                return true;
            }
            if (opcion === ltrMenus.eventosDeMf.Contabilidad.Preasientos.RegenerarLoteContable) {
                MensajesSe.Info('lotes regenerado correctamente');
                this.Recargar()
                return true;
            }
            return false;
