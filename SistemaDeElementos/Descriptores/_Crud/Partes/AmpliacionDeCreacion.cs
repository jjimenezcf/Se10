...
namespace MVCSistemaDeElementos.Descriptores
{
    ...
    public class AmpliacionDeCreacion : ControlHtml
    {
        ...
        public AmpliacionDeCreacion(IControlHtml creador, string id, string titulo, Dimension dimension, string ayuda, List<string> propiedadesNoRenderizables = null)
        : base(
          padre: creador,
          id: $"{creador.Id}_{enumTipoControl.Ampliacion}_{id}",
          etiqueta: titulo,
          propiedad:null,
          ayuda: ayuda,
          posicion: null
        ...
        public string RenderContenedorDeCuerpo()
        {
            ...
            var htmlModal = $@"<div id=¨{idHtmlCuerpoDeCreacion}¨ class=¨{enumCssEdicion.ContenedorDeAmpliacion.Render()}¨ tipo='{Tipo.Render()}' es-ampliacion='true' controlador = '{Controlador}' accion ='{Accion}' tipoDto ='{Dto.FullName}'>
                                 {DescriptorDeTabla.htmlRenderObjetoVacio(Padre, Dto, Controlador, idHtmlCuerpoDeCreacion, enumCssEdicion.TablaDeEdicion.Render(), enumModoDeTrabajo.Nuevo, Tabla.NoRenderizar)}
                               </div>";
            return htmlModal;
        }
