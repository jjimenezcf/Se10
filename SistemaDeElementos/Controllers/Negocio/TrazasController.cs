...
{
    public class TrazasController : BaseController<TrazaDto>
    {
        public TrazasController(ContextoSe contexto, IMapper mapeador, GestorDeErrores gestorDeErrores)
        : base(gestorDeErrores, contexto, mapeador)
        {
        }

        protected override IEnumerable<TrazaDto> LeerElementos(int posicion, int cantidad, List<ClausulaDeFiltrado> filtros, List<ClausulaDeOrdenacion> orden, Dictionary<string, object> opcionesDeMapeo)
        {
            ...
            var gestor = GestorDeTrazas.Gestor(Contexto, NegociosDeSe.ToEnumerado(negocioDtm.Nombre));
            
            return gestor.LeerElementos(posicion, cantidad, filtros, orden, opcionesDeMapeo);
        }

        protected override TrazaDto LeerPorId(int id, Dictionary<string, object> parametros)
        {
        ...
        public JsonResult epModificarRelacionPorPost(int idNegocio)
        {
            ...
            return ApiController.PersistirElemento(new GestorDeTrazas(Contexto, NegociosDeSe.ToEnumerado(idNegocio)),
