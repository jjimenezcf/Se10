...
namespace iText.SigningExamples.Simple
{
    class TestSignBCSimple
    {
        ...
        {
            string testDirectoryName = @"..\..\..\resources";
            string testFileName = Path.Combine(testDirectoryName, "circles.pdf");
            ...
            using (FileStream result = File.Create(Path.Combine(testDirectoryName, "circles-RSA-BC-signed-simple.pdf")))
            {
        ...
        public void TestSignSimpleDsa()
        {
            ...
            using (PdfReader pdfReader = new PdfReader(testFileName))
            using (FileStream result = File.Create(Path.Combine(testDirectoryName, "circles-DSA-BC-signed-simple.pdf")))
            {
                PdfSigner pdfSigner = new PdfSigner(pdfReader, result, new StampingProperties().UseAppendMode());
                ITSAClient tsaClient = null;
        ...
        public void TestSignSimpleECDsa()
        {
            ...
            using (PdfReader pdfReader = new PdfReader(testFileName))
            using (FileStream result = File.Create(Path.Combine(testDirectoryName, "circles-ECDSA-BC-signed-simple.pdf")))
            {
                PdfSigner pdfSigner = new PdfSigner(pdfReader, result, new StampingProperties().UseAppendMode());
                ITSAClient tsaClient = null;
